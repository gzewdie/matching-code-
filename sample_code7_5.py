...
class GlobalMinCostReconciler:
    ...
    def _build_invoice_index(self, df_invoices: DataFrame):
        ...
        ids, texts, meta = [], [], {}
        cur_bucket, ym_bucket, amt_bucket = {}, {}, {}
    ...
    def _edge_cost(self, llm_conf: float, cosine: float, amount_delta: float,
                   receipt_amount: float, days_delta: float) -> int:
    ...
    def _solve_flow(self,
        ...
        def _run_solve(use_unmatched: bool) -> List[Tuple[str,str,int]]:
            ...
            # KEY CHANGE: if unmatched sink is enabled, push ALL receipts through the network (feasible by design)
            total_send = total_receipts if use_unmatched else min(total_receipts, total_invoices)
            ...
            if _HAS_ORTOOLS:
                ...
                def nid(x):
                    nonlocal next_id
                    if x not in node_id: node_id[x] = next_id; next_id += 1
                    return node_id[x]
                ...
                if use_unmatched:
                    U = nid("U")
                    mcf.AddArcWithCapacityAndUnitCost(U, T, int(total_receipts), 0)  # allow all receipts to reach T
                    for r in receipts:
                        rnode = nid(f"R:{r['rid']}")
                ...
                mcf.SetNodeSupply(S, int(total_send))
                mcf.SetNodeSupply(T, -int(total_send))
                ...
                for i in range(mcf.NumArcs()):
                    tail = mcf.Tail(i); head = mcf.Head(i); flow = mcf.Flow(i)
                    if flow <= 0: continue
                    tn = [k for k,v in node_id.items() if v == tail][0]
                    hn = [k for k,v in node_id.items() if v == head][0]
                    if tn.startswith("R:") and hn.startswith("I:"):
                        rid = tn[2:]; inv_key = hn[2:]
        ...
        # First attempt: configured use_unmatched_sink
        flows = _run_solve(self.flow_cfg.use_unmatched_sink)
        if flows: return flows

        # Retry: force unmatched sink and try again (auto-heal)
        if self.flow_cfg.debug_metrics:
            print("[WARN] First solve infeasible; retrying with unmatched sink enabled.")
        flows = _run_solve(True)
        return flows
    ...
    def run(self, df_invoices_line_level: DataFrame, df_receipts_line_level: DataFrame) -> DataFrame:
        ...
        r_texts = [compose_receipt_text(r, self.emb_cfg.max_chars) for r in receipts_rows]
        if r_texts:
            parts = [self.emb.embed_batch(r_texts[i:i+self.emb_cfg.batch_size])
        ...
        def score_one_receipt(i: int):
            ...
            if not cand: return rid, [], {}
            ctx = {
                "receipt_id": rid,
                "amount": r.get("amount"),
                "currency": _s(r.get("currcode")) or "USD",   # default if missing
        ...
        with ThreadPoolExecutor(max_workers=self.llm_cfg.max_workers) as ex:
            ...
            for fut in as_completed(futs):
                ...
                if cand: receipt_had_candidates[rid] = True
                for c in cand:
        ...
        for i, r in enumerate(receipts_rows):
            ...
            receipts_caps.append({"rid": rid, "amount_cents": max(0, amt_c), "raw": r})
        ...
        for (rid, inv_key), feats in results_scores.items():
            if rid not in rid_set or inv_key not in invoices_caps: continue
        ...
        # 6) Solve (with unmatched retry)
        flows = self._solve_flow(receipts_caps, invoices_caps, edges_scored)

        # 7) Degrees and grouping
        rec_degree = defaultdict(int)  # invoices per receipt
        inv_degree = defaultdict(int)  # receipts per invoice
        ...
        def grouping_for(rid: str, inv_key: Optional[str]) -> str:
            if inv_key is None: return "unmatched"
            r_deg = rec_degree.get(rid, 0); i_deg = inv_degree.get(inv_key, 0)
            if r_deg == 1 and i_deg == 1: return "one-to-one"
            if r_deg > 1 and i_deg == 1:  return "one-to-many"
            if i_deg > 1:                return "many-to-one"
            return "one-to-one"

        def group_count_for(rid: str, inv_key: Optional[str]) -> int:
            if inv_key is None: return 0
            r_deg = rec_degree.get(rid, 0); i_deg = inv_degree.get(inv_key, 0)
            if r_deg == 1 and i_deg == 1: return 1
            if r_deg > 1 and i_deg == 1:  return int(r_deg)  # #invoices for this receipt
            if i_deg > 1:                return int(i_deg)  # #receipts for this invoice
            return 1

        # 8) Output rows (always one row per receipt at minimum)
        base_fields = [
            ...
            T.StructField("grouping_type", T.StringType(), True),
            T.StructField("group_count", T.IntegerType(), True),
        ...
        for rid, pairs in rid_to_flows.items():
            ...
            for inv_key, flow_cents in pairs:
                ...
                if feats:
                    ...
                    edge_cost = self._edge_cost(feats["llm_conf"], feats["cosine"],
                                                feats["amount_delta"], r_amt_f, feats["days_delta"])
                    llm_conf = feats["llm_conf"]; cosine = feats["cosine"]; llm_expl = feats["llm_explanation"]
                else:
                    edge_cost = 0; llm_conf = 0.0; cosine = 0.0; llm_expl = ""

                gtype = grouping_for(rid, inv_key)
                gcnt  = group_count_for(rid, inv_key)
                ...
                data = {
                    ...
                    "grouping_type": gtype,
                    "group_count": int(gcnt),
                    "receipt_id": _s(rc.get("receipt_id")),
                    "invoice_key": inv_key,
                }
                for c in r_cols: data[f"r_{c}"] = _safestr(rc, c)
                for c in i_cols: data[f"i_{c}"] = _safestr(ic, c)
                rows.append(Row(**data))

        # unmatched receipts → emit a row
        matched_rids = set(rid_to_flows.keys())
        for rcap in receipts_caps:
            rid = _s(rcap["rid"])
            if rid in matched_rids: continue
            rc = rcap["raw"]
            ...
            data = {
                ...
                "edge_cost": 0,
                "grouping_type": "unmatched",
                "group_count": 0,
                "receipt_id": _s(rc.get("receipt_id")),
                "invoice_key": "",
            }
            for c in r_cols: data[f"r_{c}"] = _safestr(rc, c)
            for c in i_cols: data[f"i_{c}"] = ""  # empty on unmatched
            rows.append(Row(**data))
