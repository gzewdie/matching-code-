...
# - grouping_type in {"one-to-one","one-to-many","many-to-one","unmatched"}
# - group_count rule:
#     one-to-one -> 1
#     one-to-many -> number of invoices for that receipt
#     many-to-one -> number of receipts for that invoice
#     unmatched -> 0
...
class GlobalMinCostReconciler:
    ...
    def run(self, df_invoices_line_level: DataFrame, df_receipts_line_level: DataFrame) -> DataFrame:
        ...
        inv_degree = defaultdict(int)   # number of receipts per invoice
        ...
        def group_count_for(rid: str, inv_key: Optional[str]) -> int:
            """Implements your rule:
               - one-to-one -> 1
               - one-to-many (receipt→invoices) -> number of invoices for that receipt
               - many-to-one (receipts→invoice) -> number of receipts for that invoice
               - unmatched -> 0
            """
            if inv_key is None:
                return 0
            ...
            if r_deg == 1 and i_deg == 1:
                return 1
            if r_deg > 1 and i_deg == 1:
                return int(r_deg)   # one receipt splitting to many invoices
            if i_deg > 1:
                return int(i_deg)   # many receipts feeding one invoice
            return 1
        ...
        base_fields = [
            ...
            T.StructField("grouping_type", T.StringType(), True),  # renamed
            T.StructField("group_count", T.IntegerType(), True),    # updated semantics
