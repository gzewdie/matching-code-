# faiss_llm_only_receipt_to_invoice_line_matcher_concurrent.py
# LLM-ONLY, line-level matching (Receipt lines → Invoice lines) with FAISS + concurrency.
# - Embed INVOICE LINES only, store in FAISS (cosine via inner product on normalized vectors)
...
# - Output: one best match per receipt line with llm_confidence[0..1], cosine, and short explanation
...
class LLMScorer:
    """Batched scoring: one call per receipt line, scores all invoice line candidates."""
    def __init__(self, cfg: LLMConfig):
        self.cfg = cfg
        self.client = _azure_oai_client(cfg.endpoint, cfg.api_version)

    def score_batch(self, receipt_ctx: Dict[str, Any], candidates: List[Dict[str, Any]]) -> Dict[str, Tuple[float, str]]:
        """Return { key: (confidence[0..1], explanation) }. key="invno|row_idx"""
        payload = {
            "RECEIPT_LINE": receipt_ctx,
            "CANDIDATES": [
                {"key": c["key"], "invoice_line": c["invoice_ctx"]}
                for c in candidates
            ],
            "TASK": (
                "For the single RECEIPT LINE, score each candidate INVOICE LINE independently. "
            ...
            "SCORING_GUIDE": {
                "0.95-1.0": "Amounts ~equal; dates close; same currency; customer/payer plausible; invoice/check/PO aligns.",
...
class LLMOnlyReceiptToInvoiceLineMatcherConcurrent:
    """
    LLM-only, line-level matcher (receipt line → best invoice line) with batched scoring & thread pool.

    Input DataFrames:
      - df_invoices_line_level: invno, invdate, duedate, amount/invamt, trancurr, custno, checknum, invponum, balance, ...
    ...
    Output: one best invoice line per receipt line with ids, business fields, cosine, llm_confidence, explanation.
    """
    ...
    def _build_index(self, df_invoices: DataFrame) -> Tuple[FaissIndex, Dict[str, Dict[str, Any]], int]:
        ...
        rows = [r.asDict() for r in df_invoices.select(*cols).collect()]
        ids, texts = [], []
        ...
        for j, inv in enumerate(rows):
            ...
            ids.append(key)
            meta[key] = inv
            texts.append(compose_invoice_line_text(inv, self.emb_cfg.max_chars))
        if not texts:
    ...
    def run(self, df_invoices_line_level: DataFrame, df_receipts_line_level: DataFrame) -> DataFrame:
        ...
        def process_one(idx: int) -> Dict[str, Any]:
            ...
            if not top:
                return {
                    ...
                    "llm_confidence": 0.0,
                    "cosine": None,
                    "llm_explanation": "No invoice line above cosine floor",
        ...
        with ThreadPoolExecutor(max_workers=self.llm_cfg.max_workers) as executor:
            print(f"[Receipt→Invoice LineMatcher] max_workers={getattr(executor, '_max_workers', self.llm_cfg.max_workers)}")
            futures = {executor.submit(process_one, i): i for i in range(len(rec_ids))}
            print(f"[Receipt→Invoice LineMatcher] submitted={len(futures)} tasks")
...
if __name__ == "__main__":
    ...
    # matcher = LLMOnlyReceiptToInvoiceLineMatcherConcurrent(
    #     spark,
