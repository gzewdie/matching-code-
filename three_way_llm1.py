...
_JUDGE_PROMPT = ChatPromptTemplate.from_messages([
    ...
    ("user",
     ...
     "Return JSON: {\"is_match\":true/false,\"confidence\":0-1,\"reason\":\"short\"}")
])
...
def match_receipts_to_remittances_detail(
    ...
    *,
    ...
    amt_tol_cents=0,
    date_window_days=14,
    week_bucket_radius=2,         # +/- 2 weeks supports ~14 days
    topk_per_receipt=30,
    ...
    margin=0.02,
    use_llm=True,
    llm_partitions=64,            # controls total LLM parallelism
    llm_workers_per_task=8,       # threads per Spark task
):
    def _norm(c):
    ...
    receipts_hdr = (r0.withColumn("rn", F.row_number().over(wr))
                      ...
                      .withColumn("r_amt_bucket", F.floor(F.col("r_amount_cents")/F.lit(amt_bucket_cents)).cast("long"))
    ...
    remit_hdr = (remit_hdr_base
        ...
        .withColumn("m_amt_bucket", F.floor(F.col("m_amount_cents")/F.lit(amt_bucket_cents)).cast("long"))
        .withColumn("m_week", F.floor(F.datediff(F.col("m_date"), epoch)/7).cast("int"))
    )

    # candidate join on amount bucket + week proximity
    cand = (receipts_hdr.alias("r")
        .join(remit_hdr.alias("m"),
              (F.col("r.r_amt_bucket")==F.col("m.m_amt_bucket")) &
        ...
        .filter((F.col("amt_diff") <= amt_tol_cents) & (F.col("day_diff") <= date_window_days))
...
df_out = match_receipts_to_remittances_detail(
    ...
    # optional tuning
    amt_tol_cents=0,          # set 100 for ±$1 if extraction is noisy
    date_window_days=14,
    llm_partitions=64,        # controls Spark-side parallelism for LLM calls
    llm_workers_per_task=8,   # threads per task (keep modest)
)
